...
function isIsoscelesTriangle(a, b, c) {
  const longestSide = getMaxNumber(a, b, c);
  return 2 * longestSide < a + b + c && (a === b || a === c || b === c);
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < n; i += 1) {
    let leftSide = 0;
    for (let j = 0; j < i; j += 1) leftSide += arr[j];

    let rightSide = 0;
    for (let j = i + 1; j < n; j += 1) rightSide += arr[j];
...
function getNearestBigger(number) {
  ...
  let i = d.length - 1;
  while (i >= 1 && d[i] <= d[i - 1]) i -= 1;
  if (i === 0) return number;

  let j = i;
  while (j + 1 < d.length && d[j + 1] > d[i - 1]) j += 1;
  ...
  return d.reduce((acc, v) => acc * 10 + v, 0);
}
